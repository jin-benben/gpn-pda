...
export default function Wms0017Screen() {
  ...
  const [searchText, setSearchText] = useState(
    searchParams.itemCode || searchParams.location || ""
  );
      ...
      <View className="flex-1 gap-2">
        <View className="bg-white px-2">
          ...
          <EnumSelect
            label="显示零库存："
            enumKey="yesOrNo"
            value={isShowZeroStock}
            onChange={setIsShowZeroStock}
          />
          <InputSearch
          ...
          <ScrollView>
                ...
                >
                  <View className="p-2">
                    ...
                    <View className="flex-row justify-between">
                      <Text>
                        最后上架库位：
                        {searchRes.mdm0085Info.lastPutawayLocation}
                      </Text>
                      <Text>
                        库存：
            ...
            {searchRes?.mdm0085Info && (
              <View className=" gap-2">
                {searchRes?.mdm0085Info.onHandInfo.map(
                  (a: any, index: number) => (
                    <Swipeable
                      key={index}
                      renderRightActions={(prog, drag) => {
